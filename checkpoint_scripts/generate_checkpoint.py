...
from take_checkpoint import print_tree
from take_checkpoint import deep_first_exec
...
def dump_assembly(file_path, assembly_file):
    if file_path is not None:
        with open(assembly_file, "w") as out:
            res = subprocess.run(
...
def generate_specapp_assembly(spec_base_app_list):
    for spec in spec_base_app_list:
        dst_file_path = copy_to_dst(spec, args.elfs,
...
if __name__ == "__main__":
    ...
    args = parser.parse_args()
    spec_app_list = app_list(args.spec_app_list, args.spec_apps)
    ...
    if args.archive_id is None:
        ...
        for spec_app in spec_app_list:
            prepare_rootfs(build_config()["scripts_folder"], prepare_config()["elf_folder"], spec_app)
...
#            list(map(print_tree, root_noods))
            spec_app_execute_list = root_noods
